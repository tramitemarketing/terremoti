...
(function() {
  ...
  function addMarkersToMap(events) {
    if (!mapInstance) { _pendingEvents = events; return; }
    events.forEach(ev => {
  ...
  function initMap() {
    ...
    function createMap() {
      ...
      /* se ci sono già eventi fetchati, aggiungili ora */
      if (_pendingEvents) { addMarkersToMap(_pendingEvents); _pendingEvents = null; }
    }

    /* Crea la mappa solo se la sezione ha dimensioni reali (è visibile).
       Se è ancora nascosta, aspetta che entri nel viewport. */
    if (section.getBoundingClientRect().width > 0) {
      createMap();
    } else {
  ...
  /* ── Mappa zonazione sismica (slide 5) ── */
  function initZoneMap() {
    const FILL = { 1:'#8B1A1A', 2:'#C4612A', 3:'#D4893A', 4:'#4A8A5A' };
    const ZONES = {
      ...
      .then(data => {
        L.geoJSON(data, {
          style: f => {
            ...
            return { fillColor:FILL[z], fillOpacity:0.65, color:'#1a1a1a', weight:0.8 };
          },
          onEachFeature: (f, layer) => {
            const name = f.properties.name || '';
            const z = ZONES[name.toLowerCase()] || 3;
            const label = name.charAt(0).toUpperCase() + name.slice(1);
            layer.bindTooltip(`${label} — Zona ${z}`, { sticky:true });
          }
        }).addTo(zmap);
        L.circleMarker([42.3498, 13.3995], {
          radius:5, fillColor:'#F5EDE0', fillOpacity:1, color:'#C4612A', weight:2
        }).bindTooltip("L'Aquila — Zona 1", { permanent:false }).addTo(zmap);
        zmap.fitBounds([[35.5,6.6],[47.1,18.5]]);
